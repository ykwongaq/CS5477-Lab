...
def detect_lines(image: np.ndarray) -> np.ndarray:
    ...
    """
    line_pts = np.empty(shape=[0, 3], dtype=np.float)
    """ 
    YOUR CODE HERE
    ...
    """
    edges = cv2.Canny(image, threshold1=MIN_CANNY_THRESHOLD, threshold2=MAX_CANNY_THRESHOLD)
...
def _get_lines_from_line_pts(line_pts: np.ndarray) -> np.ndarray:
    ...
    pts = np.ones([num_lines, 3], dtype=np.float)
    other_pts = np.ones([num_lines, 3], dtype=np.float)
    pts[:, :2] = line_pts[:, :2]
...
def get_pairwise_intersections(lines: np.ndarray) -> np.ndarray:
    ...
    """ YOUR CODE HERE """
    # Compute intersection point for all pairs (including pairs of itself)
    idx = 0
    for i in range(num_lines):
        for j in range(i, num_lines):
            line1 = lines[i]
            line2 = lines[j]
            intersections[idx][:] = np.cross(line1, line2)
            idx += 1
    # Remove non-existent intersections i.e. z=0
    intersections = intersections[(intersections[:, -1] != 0)]
    intersections = intersections / intersections[:, -1][:, None]
...
def get_support_mtx(intersections: np.ndarray, lines: np.ndarray, distance_threshold: float = DISTANCE_THRESHOLD):
    ...
    for intersection_idx in range(num_intersections):
        for line_idx in range(num_lines):
            ...
            line = lines[line_idx]
            d = abs(np.dot(intersection, line))
            d = d / sqrt(line[0] ** 2 + line[1] ** 2)
            support_mtx[intersection_idx][line_idx] = 1 if d < distance_threshold else 0
...
def get_vanishing_pts(lines: np.ndarray, num_vanishing_pts: int) -> np.ndarray:
    ...
    num_intersection = support_mtx.shape[0]
    zeros = np.zeros((num_intersection, 1))
    for _ in range(num_vanishing_pts):
        support_count = support_mtx.sum(axis=1)
        max_idx = np.argmax(support_count)
        vanishing_pt = intersections[max_idx]
        vanishing_pts.append(vanishing_pt)

        remove_line_idx = support_mtx[max_idx, :] == 1
        support_mtx[:, remove_line_idx] = zeros
...
def get_vanishing_line(vanishing_pts: np.ndarray):
    ...
    """ YOUR CODE HERE """
    point1 = vanishing_pts[0, :]
    point2 = vanishing_pts[1, :]
    vanishing_line = np.cross(point1, point2)
...
def _parse_info_dict(info: dict):
    ...
    top, btm = np.ones(shape=3, dtype=np.float), np.ones(shape=3, dtype=np.float)
    top[:2] = info['top']
    btm[:2] = info['bottom']
    return top, btm
...
def get_target_height(vanishing_line: np.ndarray, query_info: dict, target_info: dict, vanishing_pt_v: np.ndarray):
    ...
    # Query: Minivan
    # Target: Piller
    u = np.cross(pt_b2, pt_b1)
    u = np.cross(u, vanishing_line)
    
    l2 = np.cross(vanishing_pt_v, pt_b1)
    
    t = np.cross(pt_t2, u)
    t = np.cross(t, l2)
    t /= t[2]

    dis_from_v = distance(vanishing_pt_v, pt_b1)
    dis_from_t1 = distance(pt_t1, pt_b1)
    dis_from_t = distance(t, pt_b1)

    target_height = query_d1 * dis_from_t * (dis_from_v - dis_from_t1) / (dis_from_t1 * (dis_from_v - dis_from_t))
    """ END YOUR CODE HERE """
